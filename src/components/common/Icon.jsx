...
export default function Icon({ name, className, ...props }) {
  ...
  const fileUrl = useMemo(() => `/icons/${name}.svg`, [name]);
  useEffect(() => {
    downloadAndPrepareIcon(fileUrl).then(setIconContent);
  }, [fileUrl]);
  // eslint-disable-next-line react/no-danger
  return (<span className={mergeClassNames('inline-block', className)} dangerouslySetInnerHTML={{ __html: iconContent }} {...props} />);
}
